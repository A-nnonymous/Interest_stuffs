...
def initMat(rows, columns, density=0.4):
    a = np.random.binomial(1, density, (rows, columns))
    ini = np.mat(a)
    return ini
...
def main():
    ...
    if mode == 0:
        a = initMat(rows, columns)
        gen = input('Input generation:')
        gen = int(gen)
        for i in range(gen):
            a = evolve(a)
        print(a)
        ...
        print('Open your eyes, you stupid FUCKER!')
